...
public class BatteryService extends Service {
	...
	private String batteryPowerNo = "电池电量通知";
	// cached values
	...
	private class ScreenStateService extends BroadcastReceiver{
		...
		public void registerBatteryReceiver(boolean register, Context context) {
			...
			Log.d(TAG, "battery receiver "
					+ (register ? "ON" : "OFF (sleeping)"));
		}
	...
	private static boolean isScreenOn(Context context) {
		...
		try {
			if (sdkVersion >= 7) {
				...
				Long timeOn = (Long) service.getClass().getMethod(
						"getScreenOnTime").invoke(service);
				return timeOn > 0;
			}
		} catch (Exception e) {
