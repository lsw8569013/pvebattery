...
public class QuickBatteryActivity extends Activity implements OnClickListener,
	...
	private static final int[] LEVELS = new int[] { 100, 99, 80, 60, 40, 30,
			20, 10, 1 };
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mPrefs = getApplication().getSharedPreferences(PREFS,
	...
	protected void onResume() {
		...
		link2.setVisibility(isQuickSettingsInstalled() ? View.GONE
				: View.VISIBLE);
		...
		textView
				.setText(name == null ? getString(R.string.txt_assigned_activity_descr)
						: name);
	...
	private void updateWidgetPreview(int chargeLevel) {
		...
		String levelText = chargeLevel < 10 ? "0" + chargeLevel : String
				.valueOf(chargeLevel);
		...
		if (BatteryWidget.isCapacityRightBottom(design)) { // right-bottom
			...
			capacity_right_bottom
					.setVisibility(chargeLevel < 100 ? View.VISIBLE : View.GONE);
		} else { // center
			capacity_right_bottom.setVisibility(View.GONE);
			capacity_center.setText(levelText);
			capacity_center.setVisibility(chargeLevel < 100 ? View.VISIBLE
					: View.GONE);
		}
		...
		if (id == R.id.link1) {
			...
			try {
				Intent intent = new Intent(Intent.ACTION_VIEW, Uri
						.parse("market://search?q=" + QS_PACKAGE));
				startActivity(intent);
			} catch (Exception e) {
				e.printStackTrace();
				Toast.makeText(this, R.string.msg_no_market, Toast.LENGTH_LONG)
						.show();
			}
